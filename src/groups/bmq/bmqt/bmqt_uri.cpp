...
#define BMQT_FIX_STRINGREF(FIELD)                                             \
    ...
    BMQT_FIX_STR(__out == stream) && (!stream.bad() || old_stream.bad()) && (__out.good() || old_stream.bad())
INGREF(d_query_id)
...
UriBuilder& UriBuilder::setQualifiedDomain(const bslstl::StringRef& value)
{
    ...
    if (tier.length() > 0) {
        ...
        &__out == this && (value.find(k_TIER_PREFIX) != bslstl::StringRef::npos ==> d_uri.d_domain.length() < value.length()) && (value.find(k_TIER_PREFIX) == bslstl::StringRef::npos ==> d_uri.d_domain == value && d_uri.d_tier.empty())
d_uri.d_tier.reset();
    }
