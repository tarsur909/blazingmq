...
// requires: true
// ensures: __out != CorrelationId() && (g_id_p != 0 && (g_id_p ↦ sep_v))
...
// requires: !stream.bad()
// ensures: (stream.bad() ==> __out == stream) && (stream.good() ==> (__out == stream && (SEPFORALL(0, printer.outputSize(), i, stream + i ↦ printer.output()[i]))))
bsl::ostream&
