...
// CLASS LEVEL METHODS
(__out == true ==> SEPFORALL(0, MessageGUID::e_SIZE_HEX, i, ((buffer + i ↦ _) && ((buffer[i] >= '0' && buffer[i] <= '9') || (buffer[i] >= 'A' && buffer[i] <= 'F'))))) && (__out == false ==> SEPEXISTS(0, MessageGUID::e_SIZE_HEX, i, (buffer + i ↦ _) && (!(buffer[i] >= '0' && buffer[i] <= '9') && !(buffer[i] >= 'A' && buffer[i] <= 'F'))))
