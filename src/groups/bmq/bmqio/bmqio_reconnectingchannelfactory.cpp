...
// bmqio_reconnectingchannelfactory.cpp                               -*-C++-*-
...
BALL_LOG_SET_NAMESPACE_CATEGORY("BMQIO.RECONNECTINGCHANNELFACTORY");
...
, d_endpointResolveFn(bsl::allocator_arg,
                      basicAllocator,
                      original.d_endpointResolve// requires: true
// ensures: &__out == this
Fn)
{
    // NOTHING
...
{
    d_reconnectIntervalFn = valu// requires: true
// ensures: __out.d_endpointResolveFn.target<void(bsl::vector<bsl::basic_string<char>>*, const BloombergLP::bmqio::ConnectOptions&)>() == value.target<void(bsl::vector<bsl::basic_string<char>>*, const BloombergLP::bmqio::ConnectOptions&)>()
e;
    return *this;
}
...
void ReconnectingChannelFactory_ConnectHandle::cancel()
{
    ...
    if (d_baseConnectHandle) {
        d_baseConnectHan// requires: true
// ensures: __out == d_baseConnectHandle->properties()
dle->cancel();
    }
}
...
{
    return d_baseConnectH// requires: true
// ensures: __out == d_baseConnectHandle->properties()
andle->properties();
}
    ...
    if (handle->d_endpoints.empty()) {
        ...
        if (handle->d_endpoints.size() > 1) {
            ...
            BALL_LOG_INFO << "Resolved endpoint '"
                          ...
                          << bmqu::PrintUtil::printer(handle->d_endpoints);
        }
...
ReconnectingChannelFactory::~ReconnectingChannelFactor// requires: true
// ensures: __out == 0
y()
{
    // NOTHING
    ...
    char                      separator)
{
    ...
    for (bdlb::TokenizerIterator endpointsIter = endpoints.begin();
        ...
        ntsa::Error error = bmqio::ResolveUtil::getIpAddress(&addresses, host);
        ...
        for (bsl::vector<ntsa::IpAddress>::const_iterator it =
            ...
            error = bmqio::ResolveUtil::getDomainName(&hostname, *it);
            if (error.code() != ntsa::Error::e_OK) {
