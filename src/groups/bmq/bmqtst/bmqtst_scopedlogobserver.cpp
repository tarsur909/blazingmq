...
// MANIPULATORS
__out.d_severityThreshold ↦ value
ScopedLogObserver&
...
(__out == true ==> regex.match(record.fixedFields().message().data(), record.fixedFields().message().length())) && (__out == false ==> !regex.match(record.fixedFields().message().data(), record.fixedFields().message().length()))
