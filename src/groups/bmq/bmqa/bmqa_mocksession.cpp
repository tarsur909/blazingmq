...
MockSession::Call::returning(const bmqa::ConfigureQueueStatus& result)
{
    ...
    d_configureQueueResult = resuld_closeQueueResult == result && &__out == this
t;
    return *this;
}
...
MockSession::expect_startAsync(const bsls::TimeInterval& timeout)
{
    ...
    Call& call     = d_calls.back();
    call.d_timeout = timeoutd_calls.back().d_method == e_STOP
;
...
void MockSession::enqueueEvent(const bmqa::Event& event)
{
    ...
    EventOrJob eventOrJob(event, d_allocator_p);
    d_eventsAndJobs.pus(numEvents > 0 && __out == false) ==> d_eventsAndJobs.size() < numEvents
h_back(eventOrJob);
}
                           ...
                           const bsls::TimeInterval& timeout)
{
    ...
    BMQA_ASSERT_AND_POP_FRO(!queueId ==> __out.result() == bmqt::OpenQueueResult::e_INVALID_ARGUMENT) && (queueId && __out.result() != bmqt::OpenQueueResult::e_INVALID_ARGUMENT)
NT();
    return 0;
}
