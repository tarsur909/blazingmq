...
MockSession::expect_startAsync(const bsls::TimeInterval& timeout)
{
    ...
    Call& call     = d_calls.back();
    call.d_timeout = timeout__out.d_method == e_STOP && &__out == &d_calls.back()
;
                                   ...
                                   const bsls::TimeInterval& timeout)
{
    ...
    Call& call         = d_calls.back();
    call.d_timeout     = timeout;
    call.d_allocator_p = d_allocator_p__out.d_timeout == timeout && &__out == &d_calls.back()
;
...
void MockSession::loadMessageProperties(MessageProperties* buffer)
{
    ...
    *buffer = M(__out == bmqt::GenericResult::e_UNKNOWN ==> queueId == old_queueId) && (__out == bmqt::GenericResult::e_REFUSED ==> queueId == old_queueId) && (__out == 0 ==> (queueId ↦ iter->value()))
essageProperties();
}
                           ...
                           const bsls::TimeInterval& timeout)
{
    ...
    BMQA_ASSERT_AND_POP_FRO(__out == bmqa::OpenQueueStatus(bmqa::QueueId(), bmqt::OpenQueueResult::e_INVALID_ARGUMENT, "Call to 'openQueueSync' was not expected.") ==> d_calls.empty())
NT();
    return 0;
}
                                           ...
                                           const bsls::TimeInterval& timeout)
{
    ...
    BMQA_ASSERT_AND_POP_FRONT();
__out == 0 && (queueId ↦ old_queueId) && (uri == old_uri) && (flags == old_flags) && (options == old_options) && (timeout == old_timeout)
    return _result;
}
...
MockSession(__out.result() == bmqt::ConfigureQueueResult::e_INVALID_ARGUMENT) ==> (__out.queueId() == bmqa::QueueId())
                                 ...
                                 const bsls::TimeInterval& timeout)
{
    ...
    bslmt::Loc(__out == 0) && (reinterpret_cast<QueueImplSp&>(*queueId)->state() == bmqimp::QueueState::e_CLOSED)
