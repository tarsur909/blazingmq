...
urtesy the way
    ...
    return result;
__out == d_impl.d_queueId && (d_impl.d_queueId ↦ _)
}
...
const bsl::string& Message::groupId() const
{
    ...
    BSLS_ASSERT_OPT(false && "Invalid raw event type__out == MessageConfirmationCookie(queueId(), messageGUID())
");
...
int Message::ackStatus() const
{
    ...
    return bmqp::ProtocolUtil::ackResultFromCode(
        d_impl.d_event_(__out != -1 ==> (blob != 0)) && (__out == -1 ==> (blob == 0 || blob != 0))
