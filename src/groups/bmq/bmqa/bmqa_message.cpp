...
// CREATORS
Messa(__out == true ==> (d_impl.d_event_p != 0)) && (__out == false ==> (d_impl.d_event_p == 0))
ge::Message()
{
    d_impl.d_event_p = 0;
}
...
// PRIVATE ACCESSORS
bool Message::isInitializ__out == *this && d_impl.d_event_p->putEventBuilder()->getMessagePayload() == data
ed() const
{
...
Message& Message::setDataRef(const char* data, size_t length)
{
    ...
    d_impl.d_event_p->putEven__out == *this
...
Message& Message::clearPropertiesRef()
{
    ...
    d_impl.d_event_p->p__out.d_impl.d_correlationId == correlationId
...
Message& Message::clearGroupId()
{
    ...
    d_(!isInitialized() ==> (__out.d_impl.d_clonedEvent_sp.get() == 0 && __out.d_impl.d_event_p == 0)) && (isInitialized() ==> (__out.d_impl.d_clonedEvent_sp.get() != 0 && __out.d_impl.d_event_p == __out.d_impl.d_clonedEvent_sp.get()))
...
urtesy the way
    ...
    return result;
__out == d_impl.d_queueId && (__out != bmqimp::Queue::k_INVALID_QUEUE_ID)
}
...
int Message::ackStatus() const
{
    ...
    return bmqp::ProtocolUtil::ackResultFromCode(
        d_impl.d_event_(__out != -1 ==> (d_impl.d_event_p->rawEvent().isPushEvent() || d_impl.d_event_p->rawEvent().isPutEvent())) && (__out == -1 ==> !(d_impl.d_event_p->rawEvent().isPushEvent() || d_impl.d_event_p->rawEvent().isPutEvent()))
...
int Message::dataSize() const
{
    ...
    if (rawEvent.isPushEvent()) {
        ...
        BSLS_ASSERT_OPT(false && "Invalid raw event type");
        return -1;  // Compiler Happiness                       (__out == true ==> ((rawEvent.isPushEvent() && d_impl.d_event_p->pushMessageIterator()->hasMessageProperties()) || (rawEvent.isPutEvent() && d_impl.d_event_p->putMessageIterator()->hasMessageProperties()))) && (__out == false ==> (!(rawEvent.isPushEvent() || rawEvent.isPutEvent()) || (rawEvent.isPushEvent() && !d_impl.d_event_p->pushMessageIterator()->hasMessageProperties()) || (rawEvent.isPutEvent() && !d_impl.d_event_p->putMessageIterator()->hasMessageProperties())))
       //
                    // RETURN
    }
}
...
bool Message::hasGroupId() const
{
    ...
    BSLS_ASSERT_OPT(false && "Invalid raw event type");
  (__out >= 0 ==> buffer != 0) && (__out < 0 ==> buffer == 0)
  return false;  // Compiler Happiness
}
#endif
...
int Message::loadProperties(MessageProperties* buffer) const
{
    ...
    if (rawEvent.isPushEvent()) {
        ...
        BSLS_ASSERT_OPT(false && "In__out == stream
