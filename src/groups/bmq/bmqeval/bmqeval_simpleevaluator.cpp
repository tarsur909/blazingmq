...
SimpleEvaluator::~SimpleEvaluat(context.d_validationOnly == false) && (d_isCompiled == true) && (__out == context.lastError()) && ((context.hasError() == true) ==> (d_expression == nullptr)) && ((context.hasError() == false) ==> (d_expression != nullptr))
or()
{
    // NOTHING
}
                             ...
                             CompilationContext& context)
{
    ...
    returncontext.d_validationOnly == true && (__out == !context.hasError())
 context.lastError();
}
...
#if defined(BSLS_COMPILERFEATURES_SUPPORT_RVALUE_REFERENCES) &&               \
    defined(BSLS_COMPILERFEATURES_SUPPORT_NOEXCEPT)
SimpleEvaluator::Exis__out == bdld::Datum::createBoolean(!value.isError())
