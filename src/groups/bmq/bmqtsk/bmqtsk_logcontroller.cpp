...
BALL_LOG_SET_NAMESPACE_CATEGORY("BMQTSK.LOGCONTROLLER");
...
(__out.d_fileName ↦ rhs.d_fileName ⋆ __out.d_fileMaxAgeDays ↦ rhs.d_fileMaxAgeDays ⋆ __out.d_rotationBytes ↦ rhs.d_rotationBytes ⋆ __out.d_rotationSeconds ↦ rhs.d_rotationSeconds ⋆ __out.d_logfileFormat ↦ rhs.d_logfileFormat ⋆ __out.d_consoleFormat ↦ rhs.d_consoleFormat ⋆ __out.d_loggingVerbosity ↦ rhs.d_loggingVerbosity ⋆ __out.d_bslsLogSeverityThreshold ↦ rhs.d_bslsLogSeverityThreshold ⋆ __out.d_consoleSeverityThreshold ↦ rhs.d_consoleSeverityThreshold ⋆ __out.d_syslogEnabled ↦ rhs.d_syslogEnabled ⋆ __out.d_syslogFormat ↦ rhs.d_syslogFormat ⋆ __out.d_syslogAppName ↦ rhs.d_syslogAppName ⋆ __out.d_syslogVerbosity ↦ rhs.d_syslogVerbosity ⋆ __out.d_categories ↦ rhs.d_categories ⋆ __out.d_recordBufferSizeBytes ↦ rhs.d_recordBufferSizeBytes ⋆ __out.d_recordingVerbosity ↦ rhs.d_recordingVerbosity ⋆ __out.d_triggerVerbosity ↦ rhs.d_triggerVerbosity)
LogControllerConfig&
...
(tokens.size() != 3 ==> __out == -1) && (__out == -2 || __out == 0)
int LogControllerConfig::addCategoryProperties(const bsl::string& properties)
{
    ...
    //    stringRef because it's passed to 'bmqu::StringUtil::strTokenizeRef'
    ...
    bsl::vector<bslstl::StringRef> tokens =
        bmqu::StringUtil::strTokenizeRef(properties, ":");
...
(__out != 0 ==> d_registeredObservers.empty()) && (__out == 0 ==> SEPEXISTS(0, d_registeredObservers.size(), i, d_registeredObservers[i] ↦ observer))
                              ...
                              const LogControllerConfig& config)
{
    ...
    if (rc != 0) {
        errorDescription << "Failed enabling file logging " << "[rc: " << rc
    ...
    if (rc != 0) {
        errorDescription << "Failed registering FileObserver " << "[rc: " << rc
                         << "]";
        ball::LoggerManager::shutDownSingleton();
    ...
    if (rc != 0) {
        errorDescription << "Failed registering AlarmLog " << "[rc: " << rc
                         << "]";
        ball::LoggerManager::shutDownSingleton();
