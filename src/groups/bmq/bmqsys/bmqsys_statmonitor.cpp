...
/// the specified `accessorMethod` to extract this value. The behaviour for
/// invalid stat contexts or invalid names is undefined.
(__out == 0) ==> (!statContext.getSubcontext(subCtxName)) && (__out != 0) ==> (__out == accessorMethod(*statContext.getSubcontext(subCtxName), snapshotId, statId))
...
// MANIPULATORS
(__out != 0 ==> (errorDescription.str().find("Failed registering pid") != std::string::npos)) && (__out == 0 ==> (d_isStarted == true))
