...
__out == *str && __out == str->substr(bsl::find_if(str->begin(), str->end(), &isNotSpace) - str->begin())
...
__out == *str && __out.find_last_not_of(' ') == __out.size() - 1 || __out.empty()
...
(__out == *str) && (__out.size() <= old_str->size()) && (SEPFORALL(0, __out.size() - 1, i, !(characters.contains(__out[i]) && __out[i] == __out[i+1])))
