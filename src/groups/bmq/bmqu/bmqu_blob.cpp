...
// -----------------
// class BlobSectio(__out == stream) && (stream.bad() || (d_start ↦ _ ⋆ d_end ↦ _))
n
// -----------------
...
// --------------
// class Blob(pos.buffer() > blob.numDataBuffers() ==> __out == false) && (pos.buffer() == blob.numDataBuffers() && pos.byte() != 0 ==> __out == false) && (pos.buffer() < blob.numDataBuffers() && pos.byte() >= 0 && pos.byte() < bufferSize(blob, pos.buffer()) ==> __out == true)
Util
// --------------
                              ...
                              const BlobSection& section)
{
    ...
    if (BSLS_PERFORMANCEHINT_PREDICT_UNLIKELY(
        ...
        BSLS_PERFORMANCEHINT_UNLIKELY_HINT;
        return false;  // RETURN
   (__out == -1 ==> size == old_size) && (__out == 0 ==> (size ↦ *size ⋆ *size != old_size))
 }
                                        ...
                                        int                length)
{
    ...
    while (length != 0) {
        ...
        curPos.setBuffer(curPos.buffer() + 1);
        curP(__out == 0 ==> (*cmpResult == -1 || *cmpResult == 0 || *cmpResult == 1)) && (__out == -1 ==> *cmpResult == 0)
os.setByte(0);
    }
}
                             ...
                             int                 length)
{
    ...
    return static_cast<int>(ret == length ==> __out == 0) && (ret < 0 ==> __out == (ret * 10) - 1) && (ret > 0 && ret < length ==> __out == -2)
(outPosition - buf);
}
                         ...
                         int                 length)
{
    ...
    if (BSLS_PERFORMANCEHINT_PREDICT_LIKELY(ret == length)) {
        ...
        return ((__out == 0) ==> (ret != 0)) && ((__out == storage) ==> (ret == 0 && copyFromBlob)) && ((__out == blob.buffer(start.buffer()).data() + start.byte()) ==> (ret == 0 && bsls::AlignmentUtil::calculateAlignmentOffset(blob.buffer(start.buffer()).data() + start.byte(), alignment) == 0))
-2;  // RETURN
    }
}
