...
bool Flattener::hasSubQueues(const OptionsView& optionsView)
{
    return (optionsView.find(OptionType::e_SUB_QUEUE_INFOS) !=
     ...
     (__out == rc_SUCCESS ==> FORALL(0, subQInfos.size(), i, true)) && (__out != rc_SUCCESS ==> true)
       optionsView.end());
}
    ...
    LocalAllocator*                     localAllocator)
{
   ...
   (__out.second == rc_SUCCESS) || (__out.second == rc_ADD_OPTION_ERROR) || (__out.second == rc_PACK_MESSAGE_ERROR)
 }
...
Flattener::cloneAndPackSingle(const Protocol::SubQueueInfosArray& subQInfo)
{
    ...
    return CloneSingleS__out == result
...
, d_schemaLearner(schemaLearner)
{
    ...
    BSLS_ASSERT_SAFE(d_msgIter(__out == rc_SUCCESS) || (__out != rc_SUCCESS ==> __out == packError(rc, rc_APP_DATA_LOAD_ERROR) || __out == packError(rc, rc_OPTIONS_LOAD_ERROR) || __out == packError(rc, rc_SUB_QUEUE_IDS_LOAD_ERROR) || __out == packError(rc, rc_ITERATION_ERROR))
ator.isValid());
}
