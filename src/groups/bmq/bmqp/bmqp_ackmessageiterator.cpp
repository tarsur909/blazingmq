...
void AckMessageIterator::copyFrom(const AckMessageIterator& src)
{
...
int AckMessageIterator::next()
{
    ...
    //       'AckHeader', since first 'next()' should move beyond the
    //       'AckHeader'.
    d_advanceLength = d_header->perMessageWords() * Protocol::k_WORD_SIZE;

    // Update AckMessage, supporting protocol evolution by reading as many
                              ...
                              const EventHeader& eventHeader)
{
    ...
    // Read AckHeader, supporting protocol evolution by reading up to size of
    ...
    d_header.reset(blob,
                   d_blobIter.position(),
                   -AckHeader::k_MIN_HEADER_SIZE,
                   true,
                   false);

    if (BSLS_PERFORMANCEHINT_PREDICT_UNLIKELY(!d_header.isSet())) {
        BSLS_PERFORMANCEHINT_UNLIKELY_HINT;
        return rc_INVALID_ACKHEADER;  // RETURN
    }
    ...
    // first invocation too, by skipping over the 'AckHeader'.
...
void AckMessageIterator::dumpBlob(bsl::ostream& stream)
{
