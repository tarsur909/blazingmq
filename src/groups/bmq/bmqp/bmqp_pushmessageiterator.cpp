...
// ACC(__out == 0) ==> (!hasOptions()) && (__out != 0) ==> (hasOptions() && (__out == bmqu::BlobUtil::appendToBlob(blob, *d_blobIter.blob(), d_optionsPosition, d_optionsSize)))
ESSORS
int PushMessageIterator::loadOptions(bdlbb::Blob* blob) const
{
    ...
    return bmqu::BlobUtil::appendToBlob(blob,
                                        *d_blobIter.blob(),
                                        d_optionsPosition,
                                        d_optionsSiz(__out == -1) ==> (BSLS_PERFORMANCEHINT_PREDICT_UNLIKELY(rc != 0)) && (__out >= 0)
e);
}
...
int PushMessageIterator::loadApplicationData(bdlbb::Blob* blob) const
{
    ...
    return rc_SUCCE(__out == 0) || (__out == -1) || (__out < -2 && __out % 10 == -2) ==> true
SS;
}
...
int PushMessageIterator::loadMessageProperties(bdlbb::Blob* blob) const
{
    ...
    return rc_SUCCE(__out == 0) || (__out == -1) || (__out == -2) || (__out == -3) ==> true
SS;
}
...
int PushMessageIterator::messagePayloadSize() const
{
    ...
    return d_lazyMessagePayloadSi(__out == rc_SUCCESS) || (__out == (rc * 10 + rc_INVALID_PAYLOAD_OFFSET)) ==> (d_lazyMessagePayloadPosition == bmqu::BlobPosition() || d_lazyMessagePayloadPosition != bmqu::BlobPosition())
ze;
}
...
int PushMessageIterator:(__out == rc_HAS_NEXT) || (__out == rc_AT_END) || (__out == rc_INVALID) || (__out == rc_NO_PUSH_HEADER) || (__out == rc_NOT_ENOUGH_BYTES) || (__out == rc_INVALID_MESSAGE_SIZE) || (__out == rc_INVALID_OPTIONS_OFFSET) || (__out == rc_INVALID_APPLICATION_DATA_OFFSET) || (__out == rc_PARSING_ERROR) ==> true
:next()
{
    enum RcEnum {
        // Value for the various RC error categories
