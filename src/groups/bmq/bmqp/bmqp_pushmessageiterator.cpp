...
// ACC(__out == 0 ==> !hasOptions()) && (__out != 0 ==> hasOptions())
ESSORS
...
// ACC(isApplicationDataImplicit() ==> __out == 0) && (!isApplicationDataImplicit() ==> __out == d_applicationDataSize)
ESSORS
...
int PushMessageIterator::loadApplicationData(bdlbb::Blob* blob) const
{
    ...
    return rc_SUCCE(__out == 0) || (__out == -1) || (__out < 0)
SS;
}
...
int PushMessageIterator::loadMessageProperties(bdlbb::Blob* blob) const
{
    ...
    return rc_SUCCE(__out == rc_SUCCESS) || (__out == rc_IMPLICIT_APP_DATA) || (__out == rc_BLOB_FAILURE + 100 * __out) || (__out == rc_STREAMIN_FAILURE + 100 * __out)
SS;
}
...
int PushMessageIterator::messagePayloadSize() const
{
    ...
    return d_lazyMessagePayloadSi(__out == rc_SUCCESS) || (__out == (rc_IMPLICIT_APP_DATA * 10 + rc_INVALID_PAYLOAD_OFFSET)) || (__out == (rc_INVALID_MSG_PROPS_OFFSET * 10 + rc_INVALID_PAYLOAD_OFFSET)) || (__out == (rc_INVALID_PAYLOAD_OFFSET * 10 + rc_INVALID_PAYLOAD_OFFSET)) && (__out != rc_SUCCESS ==> d_lazyMessagePayloadPosition == bmqu::BlobPosition())
ze;
}
...
int PushMessageIterator::loadMessagePayloadPosition() const
{
    ...
    return rc_SUCCE(__out == rc_SUCCESS) || (__out == rc_IMPLICIT_APP_DATA) || (__out == rc_INVALID_PAYLOAD_OFFSET) || (__out == rc_INVALID_PAYLOAD_LENGTH)
SS;
}
...
int PushMessageIterator:(__out == 0 || __out == -1) && (__out == -1 ==> !d_blobIter.advance(eventHeader.headerWords() * Protocol::k_WORD_SIZE))
...
int PushMessageIterator:__out == 0
:reset(const bdlbb::Blob*         blob,
