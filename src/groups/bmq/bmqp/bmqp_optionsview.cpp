...
// PRIVATE ACCESSORS
(__out == 0) ==> (*payloadSizeBytes >= 0) && (__out != 0) ==> (__out == rc)
...
(__out == rc_SUCCESS ==> !subQueueInfos->empty()) && (__out == rc_LOAD_FAILURE ==> subQueueInfos->empty()) && (__out == rc_SUCCESS || __out == rc_LOAD_FAILURE || __out == rc_UNREACHABLE)
