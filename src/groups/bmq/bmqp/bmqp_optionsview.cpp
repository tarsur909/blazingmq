...
// ACCESSORS
(__out == rc_SUCCESS ==> (subQueueIdsOld != 0 && subQueueIdsOld->size() > 0)) && (__out == rc_LOAD_FAILURE ==> subQueueIdsOld->empty())
...
(__out == rc_SUCCESS) ==> (!subQueueInfos->empty()) && ((__out == rc_LOAD_FAILURE) || (__out == rc_UNREACHABLE)) ==> (subQueueInfos->empty())
