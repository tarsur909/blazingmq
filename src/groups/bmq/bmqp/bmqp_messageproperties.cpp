...
int MessageProperties::loadProperties(bool isFirstTime,
           (__out == rc_SUCCESS) || (__out == rc_DUPLICATE_PROPERTY_NAME) || (__out == d_lastError)
...
int MessageProperties::streamIn(const bdlbb::Blob& blob,
                                bool    (__out == rc_SUCCESS) || (__out == rc_MISSING_MSG_PROPERTY_HEADERS) || (__out != rc_SUCCESS && __out != rc_MISSING_MSG_PROPERTY_HEADERS)
           isNewStyleProperties)
{
    clear();
...
const bdlbb::Blob&
MessageProperties::streamOut(bdlbb::BlobBufferFactory*          bufferFactory,__out == d_blob.object() && d_isBlobConstructed == true && d_isDirty == false
