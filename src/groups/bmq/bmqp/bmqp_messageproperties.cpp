...
// -----------------------
// class MessagePropertie(property.d_value.isUnset() ==> !property.d_value.isUnset()) && (__out == property.d_value)
s
// -----------------------
...
bool MessageProperties::remove(const bsl::string&        name,
    (__out == false ==> true) && (__out == true ==> d_isDirty == true)
...
int MessageProperties::loadProperties(bool isFirstTime,
           (__out == rc_SUCCESS) || (__out == rc_DUPLICATE_PROPERTY_NAME) || (__out != rc_SUCCESS && __out != rc_DUPLICATE_PROPERTY_NAME)
...
int MessageProperties::streamIn(const bdlbb::Blob&           blob,
                              (schema != nullptr && __out == 0 ==> d_schema == schema) && (schema == nullptr ==> __out == streamIn(blob, info.isExtended()))
...
bool MessageProperties::hasProperty(const bsl::string&        name,
                             (__out == false ==> (findProperty(name) == d_properties.end())) && (__out == true ==> (findProperty(name) != d_properties.end() && (type == 0 || (*type ↦ cit->second.d_type))))
...
{
    Pro__out == d_properties.find(name)->second.d_type
...
bool MessagePropertiesIterator::hasNext()
{
    ...
    if (d_first) {
        ...
        d_first    = false;
    (__out == true ==> (d_iterator != d_properties_p->d_properties.end() && bsl::isalnum(name()[0]) && d_iterator->second.d_isValid)) && (__out == false ==> d_iterator == d_properties_p->d_properties.end())
}
    else {
