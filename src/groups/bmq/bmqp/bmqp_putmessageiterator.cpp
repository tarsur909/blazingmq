...
void PutMessageIterator::initCachedOptionsView() const
{
    ...
    // POSTCONDITIONS
    BSLS_ASSERT_SAFE(!d_optionsView.isNull((__out == -1 ==> !isValid()) && (__out >= 0 ==> true)
));
}
    ...
    bmqu::BlobPosition* position) const
{
    ...
    *position = d_applicationDataPosition;
    return rc_SUCCE(__out == rc_SUCCESS) ==> (blob != 0)
SS;
}
...
int PutMessageIterator::loadApplicationData(bdlbb::Blob* blob) const
{
    ...
    return rc_SUCCE(__out == rc_SUCCESS) || (__out == (rc_INVALID_OPTIONS_DATA * 10 + rc_INVALID_OPTIONS_DATA)) && (__out != rc_SUCCESS ==> blob != nullptr)
SS;
}
    ...
    bmqu::BlobPosition* position) const
{
    ...
    *position = d_applicationDataPosition;
    return rc_SUCCE(__out == rc_SUCCESS) || (__out != rc_SUCCESS)
SS;
}
    ...
    bmqu::BlobPosition* position) const
{
    ...
    *position = d_lazyMessagePayloadPosition;
    return rc_SUCCE(__out == 0) || (__out == -10) || (__out == -20) || (__out == -11) || (__out == -21)
SS;
}
...
int PutMessageIterator::loadMessagePayload(bdlbb::Blob* blob) const
{
    ...
    return rc_SUCCE(__out == true ==> (*msgGroupId ↦ _)) && (__out == false ==> true)
SS;
}
