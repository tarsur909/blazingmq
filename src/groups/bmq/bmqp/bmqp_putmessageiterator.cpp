...
void PutMessageIterator::initCachedOptionsView() const
{
    ...
    // POSTCONDITIONS
    BSLS_ASSERT_SAFE(!d_optionsView.isNull((__out == -1) || (__out == ((d_header.messageWords() - d_header.optionsWords() - d_header.headerWords()) * Protocol::k_WORD_SIZE - d_blobIter.blob()->buffer(lastBytePos.buffer()).data()[lastBytePos.byte()]))
));
}
...
// ACC__out == d_applicationDataSize
ESSORS
    ...
    bmqu::BlobPosition* position) const
{
    ...
    *position = d_applicationDataPosition;
    return rc_SUCCE(__out == rc_SUCCESS ==> (d_applicationDataSize > -1 || (d_applicationDataPosition != bmqu::BlobPosition() && d_applicationDataSize <= -1))) && (__out != rc_SUCCESS ==> (__out == (rc_INVALID_APP_DATA_LENGTH * 10 + rc_INVALID_APP_DATA_LENGTH)))
SS;
}
...
int PutMessageIterator::loadApplicationData(bdlbb::Blob* blob) const
{
    ...
    return rc_SUCCE(__out == 0 ==> !hasOptions() || (hasOptions() && bmqu::BlobUtil::appendToBlob(blob, *d_blobIter.blob(), d_optionsPosition, d_optionsSize) == 0)) && (__out != 0 ==> hasOptions() && bmqu::BlobUtil::appendToBlob(blob, *d_blobIter.blob(), d_optionsPosition, d_optionsSize) != 0)
SS;
}
...
int PutMessageIterator::loadOptionsView(OptionsView* view) const
{
    ...
    return view->reset(d_blobIter.blob(), d_optionsPosition, d_optionsSiz(__out == rc_NO_MSG_PROPERTIES ==> (*position == bmqu::BlobPosition())) && (__out == rc_SUCCESS ==> (*position == d_applicationDataPosition))
e);
}
    ...
    bmqu::BlobPosition* position) const
{
    ...
    *position = d_applicationDataPosition;
    return rc_SUCCE(__out == rc_SUCCESS && (!hasMessageProperties() || (hasMessageProperties() && blob != nullptr))) || (__out != rc_SUCCESS)
SS;
}
...
int PutMessageIterator::loadMessageProperties(bdlbb::Blob* blob) const
{
    ...
    return rc_SUCCE(__out == 0) || (__out == -1 + 100 * __out) || (__out == -2 + 100 * __out)
SS;
}
    ...
    MessageProperties* properties) const
{
    ...
    return rc_SUCCE__out == d_lazyMessagePayloadSize && (d_lazyMessagePayloadSize == -1 ==> __out == applicationDataSize() - messagePropertiesSize())
SS;
}
...
int PutMessageIterator::messagePayloadSize() const
{
    ...
    return d_lazyMessagePayloadSi(__out == rc_SUCCESS ==> (position ↦ d_lazyMessagePayloadPosition)) && (__out != rc_SUCCESS ==> (position ↦ bmqu::BlobPosition()))
ze;
}
...
// MANIPU__out == rc_HAS_NEXT || __out == rc_AT_END || __out == rc_INVALID || __out == rc_NO_PUTHEADER || __out == rc_NOT_ENOUGH_BYTES || __out == rc_INVALID_APPLICATION_DATA_OFFSET || __out == rc_PARSING_ERROR || __out == rc_INVALID_OPTIONS_OFFSET || __out == rc_INVALID_ADVANCE_LENGTH || __out == rc_INVALID_MESSAGE_PROPERTIES_FLAGS
LATORS
