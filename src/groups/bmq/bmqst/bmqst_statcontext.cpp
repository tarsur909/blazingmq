...
#include <bsl_vector(id.is<bsl::string>() && __out == stringHash(id.the<bsl::string>())) || (id.is<bsls::Types::Int64>() && __out == int64Hash(id.the<bsls::Types::Int64>())) || ((!id.is<bsl::string>()) && (!id.is<bsls::Types::Int64>()) && __out == 0)
.h>
...
void StatContext::moveNewSubcontexts()
{
    ...
    for (StatContextVector::iterator iter = localNewSubcontexts.begin();
        ...
        if (updates) {
            ...
            updates->resize(1 + d_update_p->subcontex(d_totalValues_p.ptr() != 0 ==> __out == d_totalValues_p.ptr()) && (d_totalValues_p.ptr() == 0 ==> __out == d_directValues_p.ptr())
ts().size());
...
, d_allocator_p(bslma::Default::allocator(basicAllocator))
{
    ...
    if (!config.d_nextSubcontextId_p.get()) {
        d_nextSubcon__out != 0 && (__out->d_uniqueId != 0) && (d_newSubcontexts.back() == __out.get())
...
void StatContext::clearSubcontexts()
{
    ...
    clearDeletedSubcontexts(0);(__out != -1 ==> SEPEXISTS(0, d_valueDefs_p->size(), i, ((*d_valueDefs_p)[i].d_name == name))) && (__out == -1 ==> SEPFORALL(0, d_valueDefs_p->size(), i, ((*d_valueDefs_p)[i].d_name != name)))
