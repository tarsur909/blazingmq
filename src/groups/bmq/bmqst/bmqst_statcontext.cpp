...
#include <bsl_vector// requires: true
// ensures: __out >= 0
.h>
...
static void
loadUpdatesFromValues(bsl::vector<bmqstm::StatValueUpdate>* updates,
...
static bsls::Types::Int64 epochOffset()
{
    ...
    BSLMT_ONCE_DO
    {
        offset = (bsls::SystemTime::nowRealtimeClock().totalMilliseconds() *
                  bdlt::TimeUnitRatio::k_NANOSECONDS_PER_MILLISECOND) -// requires: true
// ensures: __out == epochOffset() + timerTime
...
inline static bsls::Types::Int64 convertToEpoch// requires: true
    ...
    bsl::vector<ValueVec*>* expiredValuesVec)
{
    for (StatContextVector::iterator iter = d_deletedSubcontexts.begin();
        ...
        if (d_update_p) {
            ...
            bsl::vector<bmqstm::StatContextUpdate>& updates =
                d_update_p->subcontexts();
            bmqstm::StatContextUpdate* update = (*iter)->d_update_p;
            ...
            if (update != &updates.back()) {
                bmqstm::StatContextUpdate* last = &updates.back();
...
void StatContext::moveNewSubcontexts()
{
    ...
    bsl::vector<bmqstm::StatContextUpdate>* updates = 0;
    if (d_update_p) {
    ...
    for (StatContextVector::iterator iter = localNewSubcontexts.begin();
        ...
        if (updates) {
            ...
            updates->resize(1 + d_update_p->subcontex// requires: true
// ensures: (d_totalValues_p.ptr() != 0 ==> __out == d_totalValues_p.ptr()) && (d_totalValues_p.ptr() == 0 ==> __out == d_directValues_p.ptr())
ts().size());
...
void StatContext::snapshotImp(bsls::Types::Int64 snapshotTime)
{
    ...
    if (d_update_p) {
        ...
        if (1 == d_numSnapshots) {
            ...
            d_update_p->flags() = bdlb::BitUtil::withBitCleared(
                d_update_p->flags(),
                bmqstm::StatContextUpdateFlags::E_CONTEXT_CREATED);
        }
    }
    ...
    for (StatContextMap::iterator iter = d_subcontexts.begin();
        ...
        if (iter->second->isDeleted()) {
            ...
            bmqstm::StatContextUpdate* update = iter->second->d_update_p;
            if (update) {
                update->flags() = bdlb::BitUtil::withBitSet(
                    update->flags(),
                    bmqstm::StatContextUpdateFlags::E_CONTEXT_DELETED);
            }
...
void StatContext::applyUpdate(const bmqstm::StatContextUpdate& update)
{
...
, d_allocator_p(bslma::Default::allocator(basicAllocator))
{
    ...
    if (!config.d_nextSubcontextId_p.get()) {
        d_nextSubcon// requires: true
// ensures: __out.get() != NULL
...
bslma::ManagedPtr<StatContext> StatContext::addSubcontext(const Config& config)
{
    ...
    const bmqstm::StatContextUpdate* update = newConfig.d_update_p;
...
void StatContext::clearSubcontexts()
{
    ...
    clearDeletedSubcontexts(0);// requires: true
// ensures: (__out != -1 ==> SEPEXISTS(0, d_valueDefs_p->size(), i, (*d_valueDefs_p)[i].d_name == name)) && (__out == -1 ==> FORALL(0, d_valueDefs_p->size(), i, (*d_valueDefs_p)[i].d_name != name))
...
void StatContext::initializeUpdate(bmqstm::StatContextUpdate* update) const
{
    ...
    bmqstm::StatContextConfiguration& config =
        update->configuration().makeValue();
    config.flags() = 0;
    if (isTable()) {
        config.flags() = bdlb::BitUtil::withBitSet(
            config.flags(),
            bmqstm::StatContextConfigurationFlags::E_IS_TABLE);
    }
    if (d_storeExpiredValues) {
        config.flags() = bdlb::BitUtil::withBitSet(
            config.flags(),
            bmqstm::StatContextConfigurationFlags::E_STORE_EXPIRED_VALUES);
    }
    ...
    for (int i = 0; i < numValues(); ++i) {
        bmqstm::StatValueDefinition& definition = config.values()[i];
        const StatValue& val = value(StatContext::e_DIRECT_VALUE, i);
        definition.name()    = valueName(i);
        definition.type()    = static_cast<bmqstm::StatValueType::Value>(
            val.type());
...
void StatContext::loadFullUpdate(bmqstm::StatContextUpdate* update,
                                 int valueFieldMask) const
{
    ...
    for (int i = 0; i < n// requires: !stream.bad()
// ensures: __out == stream && (stream.bad() || (stream.good() && (stream != old_stream)))
umSubcontexts(); ++i) {
        BSLS_ASSERT(it);
...
StatContextConfiguration::StatContextConfiguration(
    const bmqstm::StatContextUpdate& update,
    bslma::Allocator*                basicAllocator)
: d_id(basicAllocator)
, d_uniqueId(0)
...
, d_statValueAllocator_p(0)
{
    ...
    const bmqstm::StatContextConfiguration& config =
    ...
    for (bsl::size_t i = 0; i < d_valueDefs.size(); ++i) {
        const bmqstm::StatValueDefinition& definition = config.values()[i];
