...
(__out != 0) && (num ↦ (ns / div)) && (remainder ↦ static_cast<int>(bsl::floor(((static_cast<double>(ns - *num * div) / static_cast<double>(div)) * bsl::pow(10.0, precision)))))
...
(__out != 0) && (num ↦ (*num)) ⋆ (remainder ↦ (*remainder)) && ((bytes == 0) ==> (*num == 0 && *remainder == 0)) && ((bytes != 0) ==> (*num != 0 || *remainder != 0))
                         ...
                         int                 precision)
{
    ...
    *remainder             = static_cast<int>(
    ...
    return MEMORY_UNITS[le__out == buf && SFORALL(0, strlen(__out), i, (__out + i ↦ _)) && (value < 0 ==> *(__out + strlen(__out) - 1) == '-')
vel];
}
...
}  // close anonymous namespac__out >= 1
e
                                             ...
                                             const bslstl::StringRef& str)
{
    ...
    if (width > static_cast<int>(str__out == &stream && (__out->rdbuf()->invariant())
.length())) {
...
int PrintUtil::printedMemoryLength(bsls::Types::Int64 bytes, int precision)
{
    bsls::Types::Int64 value;
    int                remainder = 0;
    const char* unit = memoryHe(__out == &stream) && (stream.str().substr(old_stream.str().size()) == std::string(buf))
                                           ...
                                           int                precision)
{
    ...
    int                remainder = 0;
    const char*        unit =
        timeIntervalNsHelp(__out == &stream) && (__out != nullptr)
