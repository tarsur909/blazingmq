...
(bytes == 0 ==> *num == 0 && *remainder == 0) && (bytes != 0 ==> *num != 0 || *remainder != 0)
                         ...
                         int                 precision)
{
    ...
    *remainder             = static_cast<int>(
    ...
    return MEMORY_UNITS[le(__out == buf) && SEPFORALL(0, strlen(buf), i, (buf + i ↦ _))
vel];
}
                                      ...
                                      bsls::Types::Int64 num)
{
    ...
    if (mod100 == 11 || mod100 == 12 || mod100 == 13) {
        return stream << "th";__out == &stream && (spacesPerLevel >= 0 ==> (__out->rdbuf()->str().size() == old_stream->rdbuf()->str().size() + stringRef.size() + 1)) && (spacesPerLevel < 0 ==> (__out->rdbuf()->str().size() == old_stream->rdbuf()->str().size() + stringRef.size()))
