...
// ------------------
// class QueueManager
// -----------(__out == QueueSp() ==> it == d_queues.end()) && (__out != QueueSp() ==> it != d_queues.end())
-------
...
QueueManager::QueueSp Qu(queue->id() >= 0 && queue != NULL) ==> d_queues.findByKey1(bmqp::QueueId(queue->id(), queue->subQueueId())) == d_queues.end()
...
QueueManager::QueueSp
QueueManager::lookupQueue(const bmqt(d_queues.findByKey2(correlationId) == d_queues.end() ==> __out.get() == 0) && (d_queues.findByKey2(correlationId) != d_queues.end() ==> __out.get() != 0)
...
unsigned int
QueueManager::subStreamCount(const bsl::strin__out == d_uris.find(canonicalUri)->second.d_subStreamCount
