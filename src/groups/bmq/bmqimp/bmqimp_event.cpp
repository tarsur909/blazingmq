...
// ensures: __out == *this && (d_type ↦ EventType::e_RAW ⋆ d_rawEvent ↦ rawEvent)
...
Event& Event::configureAsRawEvent(const bmqp::Event& rawEvent)
{
    ...
    d_type     = EventType::e_RAW;// requires: type() == EventType::e_UNINITIALIZED
// ensures: __out == *this && d_type == EventType::e_REQUEST
...
// ensures: __out == *this && d_queuesBySubscriptionId.find(SubscriptionId(queue->id(), subscriptionId)) != d_queuesBySubscriptionId.end()
