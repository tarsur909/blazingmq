...
Event& Event::configureAsRawEvent(const bmqp::Event& rawEvent)
{
    ...
    d_type     = EventType::e_RAW;(__out.d_type == EventType::e_REQUEST) ⋆ (__out.d_eventCallback != nullptr)
                               ...
                               const bslstl::StringRef&     errorDescription)
{
    ...
    d_errorDesc(__out.type() == EventType::e_MESSAGE) && (__out.d_msgEventMode == MessageEventMode::e_READ)
...
Event& Event::configureAsMessageEvent(const bmqp::Event& rawEvent)
{
    ...
    d_rawEvent     = rawEv__out.d_type == EventType::e_MESSAGE && __out.d_msgEventMode == MessageEventMode::e_WRITE && __out.d_isPutEventBuilderConstructed == true && (__out.d_putEventBuilderBuffer.buffer() ↦ _)
ent;
...
Event& Event::insertQueue(const bsl::shared_ptr<Queue>& queue)
{
    ...
    d_queues.insert(bsl::(__out.d_queuesBySubscriptionId.find(SubscriptionId(queue->id(), subscriptionId)) != __out.d_queuesBySubscriptionId.end()) && (__out.d_queuesBySubscriptionId.find(SubscriptionId(queue->id(), subscriptionId))->second == queue) && (&__out == this)
                           ...
                           const bmqt::CorrelationId&    corrId)
{
    ...
    if (!corrId.isUnset()) {
        ...
        d_messageCorrelati__out == stream
